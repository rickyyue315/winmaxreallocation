...
def create_om_transfer_chart(recommendations_df):
    ...
    bars1 = ax.bar(x - width/2, transfer_by_om.values, width, 
                   label='Transfer Out', color='#4682B4', alpha=0.8)  # 青藍色
    bars2 = ax.bar(x + width/2, receive_by_om.values, width, 
                   label='Receive In', color='#20B2AA', alpha=0.8)  # 青綠色
    
    # 設置圖表
    ax.set_xlabel('OM Unit', fontsize=12)
    ax.set_ylabel('Transfer Quantity', fontsize=12)
    ax.set_title('Transfer Out vs Receive In Quantity by OM', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
        ...
        if df is not None:
            ...
            if st.button("🚀 開始分析", type="primary"):
                ...
                if summary_stats['total_recommendations'] > 0:
                    ...
                    # 添加條形圖顯示
                    st.write("**📊 Transfer Out vs Receive In Quantity Chart by OM**")
                    try:
