...
### 🆕 C模式（重點補0）新功能
- 🎯 **新增極低庫存補充模式**
  - 觸發條件：(SaSa Net Stock + Pending Received) ≤ 1
  - 補充策略：補充至 min(Safety Stock, MOQ + 1) 數量
  - 優先處理：僅針對RF類型商品，按銷量排序（高銷量優先）
  - 調貨類型：'重點補0' (Critical Zero Restock)

- 🔧 **C模式專用功能**
  - 新增 `identify_receive_candidates_mode_c()` 專用函數
  - 智能補充目標計算：取安全庫存與MOQ+1的較小值
  - 高銷量產品優先補充，確保熱銷商品不斷貨
  - 完整的C模式統計預覽和視覺化支援

### 🧪 測試驗證結果
- ✅ 模擬測試：3個極低庫存案例全部正確識別
  - A001(庫存0+待收1=1): 補充9件至安全庫存10
  - A002(庫存1+待收0=1): 補充5件至MOQ+1=6
  - A003(庫存0+待收0=0): 補充7件至MOQ+1=7
- ✅ 真實數據測試：SUI LIP GU.xlsx 中發現1個重點補0需求
  - HBA4-109801606001: 庫存1，需補充6件（高銷量28件/月）

### 📈 業務價值
- **零庫存風險控制**：主動識別極低庫存風險（≤1件）
- **智能補充策略**：根據安全庫存和MOQ制定合理補充量
- **銷量導向優先級**：優先處理高銷量商品，減少缺貨損失
- **運營效率提升**：自動化極低庫存監控和補充建議

### 📚 系統界面更新
- 🖥️ **三模式選擇**：A(保守轉貨) / B(加強轉貨) / C(重點補0)
- 📊 **三模式統計預覽**：顯示A/B/C模式的預計轉出/接收/需求數量
- 🎨 **視覺化增強**：支援'重點補0'類型的圖表顯示
- 📱 **幫助文字優化**：清晰說明三種模式的適用場景和策略

### 📚 技術實現
- 新增 `test_c_mode_v173.py` C模式功能測試
- 新增 `test_real_data_c_mode.py` 真實數據C模式測試
- 更新主程序支援C模式的完整流程集成
- 完整的C模式邏輯測試和驗證
...
### 🔧 模式B同店舖同SKU衝突修正
- 🚫 **解決同店舖同SKU轉出/接收矛盾問題**
  - 修正模式B中同一店舖對同一SKU既轉出又接收的邏輯矛盾
  - 新增 `resolve_same_store_conflicts()` 函數進行衝突檢測和解決
  - 衝突解決策略：優先保持接收需求，移除相衝突的轉出候選
  - 確保每個SKU在每個店舖只有單一的調貨動作（轉出或接收）

- 🎯 **衝突解決邏輯**
  - 檢測條件：同一店舖(Site) + 同一產品(Article) + 同一OM
  - 優先級策略：接收需求 > 轉出候選（保障庫存安全）
  - 自動移除衝突的轉出候選，保持所有接收候選不變
  - 詳細記錄衝突解決過程，提供透明的處理追蹤

### 🧪 測試驗證結果
- ✅ 測試案例：Store A - TEST001 同時轉出7件(RF過剩轉出)和接收5件(SasaNet調撥接收)
- ✅ 衝突解決：自動移除Store A的轉出7件，保持接收5件需求
- ✅ 最終結果：Store A僅接收，Store B提供轉出，邏輯完全一致
- ✅ 其他無衝突的轉出候選正常保留

### 📈 業務價值
- **邏輯一致性**：消除同店舖同SKU的調貨矛盾
- **庫存安全**：優先保障庫存不足的接收需求
- **運營效率**：避免不必要的同店舖內部調貨
- **決策透明**：提供清晰的衝突解決記錄

### 📚 技術實現
- 新增 `test_v172_conflict_resolution.py` 全面測試腳本
- 修改 `generate_transfer_suggestions()` 添加衝突解決步驟
- 新增衝突檢測和解決的詳細日誌輸出
- 完整的衝突解決邏輯測試和驗證
